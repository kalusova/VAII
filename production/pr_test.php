...
<title>Moto-StyleMX Production</title>
<link href="http://www.moto-stylemx.com/eshop/templates/style-mx/favicon.ico" rel="shortcut icon" type="image/x-icon" />
</head>
<body>
...
function zmena(id, p) {
	...
	if (priority_val) { $('#priority_img_'+id).attr("src", "images/red.png"); }
	else { $('#priority_img_'+id).attr("src", "images/green.png"); }	

	$.post("save.php", {id: id, priority: priority_val, design: design_val, model: model_val, format: format_val, status: status_val, ebay: ebay_val, notes: notes_val, payment: payment_val, change_status: change_status_val}, function(data){
		$('#db_status').html(data);
...
// echo $_COOKIE["f_none"]." ".$_COOKIE["f_printed"]." ".$_COOKIE["f_up"]." ".$_COOKIE["f_done"];
?>
...
<tr>
<td class="nadpis" width="40">Prior.</td>
<td class="nadpis" width="200">Design No.</td>
...
<td class="nadpis" width="110">eBay</td>
<td class="nadpis" width="600">Notes</td>
...
<td class="<?php  echo $class; ?>"><input autocomplete="off" id="design_<?php  echo $row['id']; ?>" onchange="zmena(<?php  echo $row['id']; ?>);" maxlength="20" class="<?php  echo $class; ?>" type="text" value="<?php echo $row['design']; ?>"></td>
...
mysql_close($con);
?>
<span class="db">&copy; 2012 <a href="http://www.norwes.eu/" STYLE="text-decoration:none;">Norwes Technologies, Ltd.</a> - Version: 1.8.0</span>
<?php ob_end_flush(); ?>
</body>
</html>
